...
export function MobileMenu({ navigation, pathname, isOpen, onClose }: Props) {
  ...
  return (
    <div
      className={`fixed top-0 left-0 w-full h-screen bg-white p-4 flex flex-col items-start pt-20 z-50 transition-transform duration-300 md:hidden ${
            ...
            >
              {hasSubmenu ? (
                <>
                  ...
                  >
                    <span className="text-lg text-gray">{group.menuItem.title}</span>
                    {isSubmenuOpen ? <MdClose size={20} className="text-gray" /> : <ChevronArrow />}
                  </button>
                  <div
                    ...
                    <div className="flex flex-col items-center">
                      {group.submenuItem.map((item, i) => (
                        <a
                          ...
                          href={item.link}
                          onClick={onClose}
                          className={`text-sm tracking-wider py-2 transition-colors duration-300 ${
                            pathname === item.link
                ...
                <button
                  className="text-lg text-gray w-full text-center py-4"
                ...
                <a
                  ...
                  className="text-lg text-gray w-full text-center py-4 no-underline"
                  onClick={onClose}
                >
                  {group.menuItem.title}
                </a>
